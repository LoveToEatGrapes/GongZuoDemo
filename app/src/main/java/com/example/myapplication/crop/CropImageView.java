...
import com.example.myapplication.crop.edge.Edge;
import com.example.myapplication.crop.handle.Handle;
import com.example.myapplication.crop.util.AspectRatioUtil;
import com.example.myapplication.crop.util.HandleUtil;
import com.example.myapplication.crop.util.PaintUtil;
...
public class CropImageView extends ImageView {
    ...
    public Bitmap getCroppedImage() {
        ...
        //提取比例和平移值。请注意，我们目前不处理其他任何转换（例如倾斜）。
        ...
        //确保右边缘和下边缘不在ImageView范围之内（这只是为了解决舍入差异）
...
//        final float cropHeight = Math.abs(Math.min(Edge.getHeight() / scaleY, originalBitmap.getHeight() - cropY));

        Log.e("TAG","==========================:" + Edge.getHeight() / scaleY + "==" +
                (originalBitmap.getHeight() - cropY));

        Log.e("TAG","=============bitmap高度：" +originalBitmap.getHeight() + "=cropY=" + cropY);

        Log.e("TAG","-----------::" + "=cropX=" + cropX + "=cropY="
                + cropY + "=cropWidth=" + cropWidth + "=cropHeight=" + cropHeight
             + "=abs=" + Math.abs(cropHeight));
     ...
     * 如果关闭了固定的宽高比，则初始裁切窗口将设置为显示的*图像，其边距为10％。
     * 如果启用了固定的长宽比，则初始裁剪窗口将*符合长宽比，并且至少将一个尺寸最大化。
     */
    private void initCropWindow(@NonNull RectF bitmapRect) {
        ...
        if (mFixAspectRatio) {
            ...
            // 初始化裁剪窗口，使其相对于Drawable边界具有10％的填充。
